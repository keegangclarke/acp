...
rm_chunks = [*chunks(remainder,6)]

# %%
# Data Wrangling
for name in rm_chunks[1]: #remainder
    # select data
    MKT = name #'SPX.Index' 
    ...
    d_combined = "C:/Users/Keegan/OneDrive/1 Studies/2021 - 2022/5003W/3 - Dissertation/5-Data/results/fanomal/stats_models/combined/"+MKT+"/"
    d_trade = d_ae+'trade/'

    # Store results in place for comparison
    d_comparison = "C:/Users/Keegan/OneDrive/1 Studies/2021 - 2022/5003W/3 - Dissertation/5-Data/results/fanomal/stats_models/comparison_folders/"
    d_distributions = d_comparison+"distributions/"
    d_full_sample = d_comparison+"full_sample/"
    d_ae_close_up = d_comparison+"ae_close_up/"
    d_ae_error_dist= d_comparison+'ae_error_dist/'
    d_combined_error = d_comparison+'combined_error_dists/'
    d_rmse = d_comparison+'ae_rmse_dist/'
    d_garch_full_sample = d_comparison+'garch_full_sample/'
    d_garch_close_up = d_comparison+'garch_close_up/'
    d_garch_vs_arima = d_comparison+'garch_vs_arima/'
    d_ar_covid = d_comparison+'covid_period/'
    d_acp_omnibus = d_comparison+'omnibus_plot/'
    ...
    # comparison_folder
    Path(d_comparison).mkdir(parents=True, exist_ok=True)
    Path(d_distributions).mkdir(parents=True, exist_ok=True)
    Path(d_full_sample).mkdir(parents=True, exist_ok=True)
    Path(d_combined_error).mkdir(parents=True, exist_ok=True)
    Path(d_garch_full_sample).mkdir(parents=True, exist_ok=True)
    Path(d_garch_close_up).mkdir(parents=True, exist_ok=True)
    Path(d_garch_vs_arima).mkdir(parents=True, exist_ok=True)
    Path(d_ae_close_up).mkdir(parents=True, exist_ok=True)
    Path(d_ae_error_dist).mkdir(parents=True, exist_ok=True)
    Path(d_rmse).mkdir(parents=True, exist_ok=True)
    Path(d_ar_covid).mkdir(parents=True, exist_ok=True)
    Path(d_acp_omnibus).mkdir(parents=True, exist_ok=True)
   ...
   # .______________________________.
    ...
    # Insert Title
    fig.suptitle('Dispersion Estimator Full Sample: '+str(MKT).replace('.',' '), y = 0.98, size=TITLE_FONT['size']) #fontsize=TITLE_FONT['size']
    fig.subplots_adjust(hspace=0.4, top=0.90)

    # Axis 1
    ax1.plot(y_vol.rolling(window=40).mean(), label='MA(40)', linewidth=2, alpha=1, color='violet', zorder=0)

    ax1.plot(svol, label='Train Conditional Volatility', linewidth=1, path_effects = PE, zorder=10) #.divide(SCALE).pow(2)
    ax1.plot(fvol, label='Forecast Conditional Volatility', linewidth=1, path_effects = PE, zorder=9) #.divide(SCALE).pow(2)
    ax1.scatter(x=y_vol.index, y=y_vol.values, label='Actual Vol estimate', s=20, alpha=0.5, zorder=1)
    # ax1 settings
    ax1.set_title('GJR-GARCH(1,1,1)', fontdict=AXIS_FONT)
    # ax1.title.set_text('GARCH(1,1,1) fitted: '+str(MKT).replace('.',' '), fontsize=AXIS_FONT['size']) #fontsize=TITLE_FONT['size']
    # ax1.legend()
    ...
    # Axis 2
    ax2.plot(garch_resids, label='Garch Residuals', alpha=1, zorder=2, color="indianred")
    # ax2 settings
    ...
    ax2.set_title('GJR-GARCH(1,1,1) residuals ', fontdict=AXIS_FONT)
    ...
    if save_figs:
        ...
        fig.savefig(d_garch_full_sample+MKT+'__GRJ-GARCH(1,1,1)_in-sample_vs_out-sample.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
    else:
        if verbose:
            print('Not storing plot.')
    ...
    # Insert Title
    fig.suptitle('Dispersion Estimator Close Up: '+str(MKT).replace('.',' '), y = 0.98, size=TITLE_FONT['size']) #fontsize=TITLE_FONT['size']
    fig.subplots_adjust(hspace=0.4, top=0.90)

    # Axis 1
    ax1.plot(y_vol[(y_vol.index>LOWER)&(y_vol.index<UPPER)].rolling(window=40).mean(), label='MA(40)', linewidth=2, alpha=1, color='violet', zorder=0)

    ax1.plot(svol[(svol.index>LOWER)&(svol.index<UPPER)], label='Train Conditional Volatility', linewidth=1, path_effects = PE, zorder=10) #.divide(SCALE).pow(2)
    ax1.plot(fvol[(fvol.index>LOWER)&(fvol.index<UPPER)], label='Forecast Conditional Volatility', linewidth=1, path_effects = PE, zorder=9) #.divide(SCALE).pow(2)
    ax1.scatter(x=y_vol[(y_vol.index>LOWER)&(y_vol.index<UPPER)].index, y=y_vol[(y_vol.index>LOWER)&(y_vol.index<UPPER)].values, label='Actual Vol estimate', s=20, alpha=0.5, zorder=1)
    # ax1 settings
    ax1.set_title('GJR-GARCH(1,1,1)', fontdict=AXIS_FONT)
    ax1.axvspan(xmin=E1,xmax=E4, alpha=0.2, color='black')
    ax1.set_facecolor('whitesmoke')
    ...
    # Axis 2
    ax2.plot(garch_resids[(garch_resids.index>LOWER)&(garch_resids.index<UPPER)], label='Garch Residuals', alpha=1, zorder=2, color="indianred")
    # ax2 settings
    ax2.set_facecolor('whitesmoke')
    ax2.margins(x=0)
    ax2.xaxis.set_major_formatter(DATE_FORM)
    ...
    ax2.set_xlabel('Date',fontdict=AXIS_FONT)
    ax2.axvspan(xmin=E1,xmax=E4, alpha=0.2, color='black')
    ax2.set_title('GARCH(1,1,1) residuals ', fontdict=AXIS_FONT)

    # Axis 3
    ax3.plot(prices[(prices.index>LOWER)&(prices.index<UPPER)], label=str(MKT).replace(".", " "), alpha=1, zorder=2, color="goldenrod")
    # ax3 settings
    ax3.set_facecolor('whitesmoke')
    ax3.margins(x=0)
    ax3.xaxis.set_major_formatter(DATE_FORM)
    ...
    if save_figs:
        fig.savefig(d_garch_close_up+MKT+'__GJR-GARCH(1,1,1)_close-up.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
    else:
        if verbose:
            print('Not storing plot.')
    # %%
    ...
    plt.grid(visible=True, which='both', axis='both', color='white', alpha=1, linewidth=1.5)
    fig.suptitle('Residual Distributions: '+str(MKT).replace('.',' '), fontproperties=TITLE_FONT)
    ...
    if save_figs:
        ...
        fig.savefig(d_garch_vs_arima+'ARIMA(1,1,1)_vs_GRJ-GARCH(1,1,1)_residual distributions.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
    else:
        if verbose:
            print('Not storing plot.')
    ...
    run = wandb.init(
        ...
        save_code=True,
        reinit=True,
        mode='offline', # options are ['online', 'offline', 'disabled']
    )
    config = run.config
    ...
    if save_figs:
        ...
        fig.savefig(d_ae_error_dist+str(MKT).replace('.',' ')+'__AE_reconstruction_distributions.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
    ...
    if save_figs:
        ...
        fig.savefig(d_combined_error+str(MKT).replace('.',' ')+'__ARIMA(1,1,1)_vs_GRJ-GARCH(1,1,1)_vs_AE__residual_distributions.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
    else:
        if verbose:
            print('Not storing plot.')
    ...
    if save_figs:
        ...
        fig.savefig(d_rmse+str(MKT).replace('.',' ')+'mse_loss_distributions.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
    else:
        if verbose:
            print('Not storing plot.')
    ...
    # Settings
    ax2.title.set_text('AE RMSE vs AE Reconstruction Inaccuracy')
    ax2.title.set_fontweight('semibold')
    ax2.set_facecolor('whitesmoke')
    ax2.xaxis.set_major_formatter(DATE_FORM)
    ax2.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ...
    # Insert heading
    fig.suptitle('Covid Period: '+str(MKT).replace('.',' '), y = 0.9, size=TITLE_FONT["size"])    
    fig.subplots_adjust(hspace=0.4, top=0.85)
    ...
    ax2.axvspan(xmin=E1,xmax=E4, alpha=0.1, color='black')
    # Vertical lines
    ax2.vlines(E1, ymin=rmse_slice.min(), ymax=rmse_slice.max(), color='black', linewidth=1)
    ax2.vlines(E2, ymin=rmse_slice.min(), ymax=rmse_slice.max(), color='black', linewidth=1)
    ax2.vlines(E3, ymin=rmse_slice.min(), ymax=rmse_slice.max(), color='black', linewidth=1)
    ax2.vlines(E4, ymin=rmse_slice.min(), ymax=rmse_slice.max(), color='black', linewidth=1)
    ...
    # Axis 3
    ax3.plot(reshifted_slice, color='darkred', label='AE Reconstruction Accurary', linewidth=1.5)
    ax3.axvspan(xmin=E1,xmax=E4, alpha=0.1, color='black')
    # Vertical lines
    ax3.vlines(E1, ymin=reshifted_slice.min(), ymax=reshifted_slice.max(), color='black', linewidth=1)
    ax3.vlines(E2, ymin=reshifted_slice.min(), ymax=reshifted_slice.max(), color='black', linewidth=1)
    ax3.vlines(E3, ymin=reshifted_slice.min(), ymax=reshifted_slice.max(), color='black', linewidth=1)
    ax3.vlines(E4, ymin=reshifted_slice.min(), ymax=reshifted_slice.max(), color='black', linewidth=1)

    ax3.text(E1, reshifted_slice.max(), 'E1', horizontalalignment='right', verticalalignment='top', fontsize=10, fontweight='semibold')
    ax3.text(E2, reshifted_slice.max(), 'E2', horizontalalignment='right', verticalalignment='top', fontsize=10, fontweight='semibold')
    ax3.text(E3, reshifted_slice.max(), 'E3', horizontalalignment='right', verticalalignment='top', fontsize=10, fontweight='semibold')
    ax3.text(E4, reshifted_slice.max(), 'E4', horizontalalignment='right', verticalalignment='top', fontsize=10, fontweight='semibold')
    # Settings
    ...
    # Axis 4
    ax4.plot(price_slice, color='orange', label=str(MKT).replace('.',' '), linewidth=2)
    ax4.axvspan(xmin=E1,xmax=E4, alpha=0.1, color='black')
    # Vertical lines
    ax4.vlines(E1, ymin=price_slice.min()[0], ymax=price_slice.max()[0], color='black', linewidth=1)
    ax4.vlines(E2, ymin=price_slice.min()[0], ymax=price_slice.max()[0], color='black', linewidth=1)
    ax4.vlines(E3, ymin=price_slice.min()[0], ymax=price_slice.max()[0], color='black', linewidth=1)
    ax4.vlines(E4, ymin=price_slice.min()[0], ymax=price_slice.max()[0], color='black', linewidth=1)

    ax4.text(E1, price_slice.max()[0], 'E1', horizontalalignment='right', verticalalignment='top', fontsize=10, fontweight='semibold')
    ax4.text(E2, price_slice.max()[0], 'E2', horizontalalignment='right', verticalalignment='top', fontsize=10, fontweight='semibold')
    ax4.text(E3, price_slice.max()[0], 'E3', horizontalalignment='right', verticalalignment='top', fontsize=10, fontweight='semibold')
    ax4.text(E4, price_slice.max()[0], 'E4', horizontalalignment='right', verticalalignment='top', fontsize=10, fontweight='semibold')
    # Settings
    ax4.title.set_text(str(MKT).replace('.',' '))
    ax4.title.set_fontweight('semibold')
    ax4.set_ylim(ymin=price_slice.min()[0])
    ...
    if save_figs:
        fig.savefig(d_ae_plots+'AE_covid-period.png', dpi=600, facecolor='white', edgecolor='white', bbox_inches='tight', pad_inches=0.1)
        fig.savefig(d_ar_covid+str(MKT).replace('.',' ')+'_AE_covid-period.png', dpi=600, facecolor='white', edgecolor='white', bbox_inches='tight', pad_inches=0.1)
    ...
    # Insert Title
    fig.suptitle('Full Sample: '+str(MKT).replace('.',' '), y = 0.98, size=TITLE_FONT['size'])
    fig.subplots_adjust(hspace=0.4, top=0.90)
    ...
    # Axis 3
    ax3.plot(reshifted_rmse_loss, color='darkred', label='AE Reconstruction inaccuracy', linewidth=1)
    ax3.axvspan(xmin=E1,xmax=E4, alpha=0.1, color='black')
    # Settings
    ax3.set_facecolor('whitesmoke')
    ax3.margins(x=0)
    ...
    ax3.yaxis.set_major_formatter(FormatStrFormatter('%.3f'))
    ax3.set_xlabel('Date',fontdict=AXIS_FONT)
    ax3.title.set_text('AE Reconstruction Inaccuracy')
    ax3.title.set_fontweight('semibold')
    # ax2.legend()

    # # Axis 4
    # ax4.plot(prices, color='orange', label=str(MKT).replace('.',' '), linewidth=2)
    # ax4.axvspan(xmin=E1,xmax=E4, alpha=0.1, color='black')
    # # Settings
    # ax4.title.set_text(str(MKT).replace('.',' '))
    # ax4.title.set_fontweight('semibold')
    ...
    if save_figs:
        ...
        fig.savefig(d_full_sample+str(MKT).replace('.',' ')+'_AE_full-sample_preds.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
    else:
        if verbose:
            print('Not storing plot.')
    ...
    ax2.plot(reshifted_rmse_loss, color='darkred', label='AE Reconstruction Inaccuracy', linewidth=1.5)
    ...
    if save_figs:
        fig.savefig(d_model_results+str(MKT).replace('.',' ')+'_omnibus_plot.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
        fig.savefig(d_acp_omnibus+str(MKT).replace('.',' ')+'_omnibus_plot.png', dpi=600, facecolor='white', edgecolor='white',bbox_inches = 'tight', pad_inches = 0.1)
    else:
        if verbose:
            print('Not storing plot.')
    ...
    trade['Mean'] = train_rmse_loss.mean()
    trade['Mean'] = train_rmse_loss.median()
    trade['Train 1 STD'] = train_rmse_loss.mean() + 1*train_rmse_loss.std()
